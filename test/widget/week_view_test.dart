...
void main() {
  ...
  Future<void> seedWeekTasks(WidgetTester tester, ProviderContainer container) async {
    final categories = await runDb(tester,
    ...
    Future<void> insert(int dayOffset, int hour, String title,
  ...
  Finder weekColumn(String iso) =>
      find.byKey(ValueKey('week-column-$iso'));
  ...
  testWidgets('W opens the Week View with seven day columns', (tester) async {
    ...
    expect(
        find.byKey(ValueKey(
            'day-count-${isoDateString(monday())}')),
        findsOneWidget);
    ...
    final decoration = tester
        .widget<Container>(blockFinder.first)
        .decoration as BoxDecoration;
    expect(decoration.border, isA<Border>());
    expect((decoration.border as Border).left.color,
        AppColors.parseHex('#4285F4'));
    await finish(tester, container);
  });

  testWidgets('tapping a day column navigates to its Day View',
      (tester) async {
    final container = await pumpDay(tester);
    await seedWeekTasks(tester, container);
    await openWeekView(tester);
    ...
    expect(
        find.text(DateFormat('MMM d, yyyy').format(tuesday)), findsOneWidget);
  ...
  testWidgets('W toggles back to Day View and week navigation works',
    ...
    await settle(tester);
    expect(container.read(selectedWeekStartProvider),
