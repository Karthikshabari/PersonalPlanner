...
void main() {
  ...
  Future<void> seedTwoTasks(
      WidgetTester tester, ProviderContainer container, DateTime date) async {
  ...
  Future<void> showReviewForToday(WidgetTester tester,
  ...
  testWidgets('fill and save the daily review persists it and refreshes stats',
      (tester) async {
    final container = await pumpReview(tester);

    await tester.enterText(
        find.byKey(const ValueKey('review-reflection')), 'Solid focus day');
  ...
  testWidgets('weekly review shows aggregates and saves the review',
    ...
    await tester.enterText(
        find.byKey(const ValueKey('weekly-reflection')), 'Good week overall');
    ...
    final repo = container.read(reviewRepositoryProvider);
    final saved = await runDb(tester,
