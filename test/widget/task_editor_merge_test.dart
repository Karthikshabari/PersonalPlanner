...
void main() {
  ...
  Future<void> save(WidgetTester tester) async {
    ...
    final editorScrollable =
        find.ancestor(of: button, matching: find.byType(Scrollable)).last;
    for (var attempt = 0; attempt < 12; attempt++) {
      await settle(tester);
      final rect = tester.getRect(button);
      final viewport = tester.getRect(editorScrollable);
      if (rect.top >= viewport.top + 24 && rect.bottom <= viewport.bottom - 24) {
        break;
      }
  ...
  testWidgets('save merges unrelated external changes', (tester) async {
    ...
    await runDb(
      tester,
      () => container.read(taskRepositoryProvider).updateTask(
            current!.copyWith(description: 'External description'),
          ),
    );
    await save(tester);
  ...
  testWidgets('same-field conflict offers reload and keeps latest values',
    ...
    await runDb(
      tester,
      () => container.read(taskRepositoryProvider).updateTask(
            current!.copyWith(title: 'Remote title'),
          ),
    );
    await save(tester);
