...
void main() {
  ...
  group('SubtaskRepository', () {
    test('insert, watch ordered by sortOrder, toggle, soft-delete', () async {
      ...
      final a = await subtasks.insertSubtask(Subtask(id: '', taskId: task.id,
          title: 'A', sortOrder: 0, createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final b = await subtasks.insertSubtask(Subtask(id: '', taskId: task.id,
          title: 'B', sortOrder: 1, createdAt: DateTime.now(), updatedAt: DateTime.now()));
      ...
      final raw = await (db.select(db.subtasks)
            ..where((s) => s.id.equals(b.id)))
          .getSingle();
    ...
    test('reorderSubtasks persists the given order', () async {
      ...
      for (final title in ['A', 'B', 'C']) {
        final s = await subtasks.insertSubtask(Subtask(
            id: '',
            taskId: task.id,
            title: title,
      ...
      final order =
          (await subtasks.getSubtasksForTask(task.id)).map((s) => s.title);
      expect(order, ['B', 'C', 'A']);
    });
  });
  ...
  group('TagRepository', () {
    ...
    test('attach/detach tags on a task; watchTagsForTask streams them',
        () async {
      final task = await seedTask();
      ...
      await expectLater(tags.watchTagsForTask(task.id).first,
  ...
  group('InboxRepository', () {
    test('addToInbox creates an unscheduled inbox item surfaced by the stream',
        () async {
    ...
    test('overdue scheduled tasks surface alongside inbox items with stamping',
        () async {
      final yesterday =
      ...
      final items = await inbox.watchInboxItems().first;
      expect(items.map((i) => i.task.title), containsAll(['Idea', 'Past thing']));
      final overdue =
          items.firstWhere((i) => i.task.title == 'Past thing');
    ...
    test('scheduleItem clears the inbox flag and sets times', () async {
      ...
      final result =
          await inbox.scheduleItem(item.id, start, start.add(const Duration(hours: 1)));
    ...
    test('rescheduleOverdue creates linked copy and marks original',
        () async {
      ...
      final copy = await inbox.rescheduleOverdue(
          original.id, newStart, newStart.add(const Duration(hours: 1)));
  ...
  group('CategoryRepository CRUD support', () {
    ...
    test('reorderCategories persists sort order', () async {
      final c1 = await categories.insertCategory(Category(
          id: '', name: 'Zed', colorHex: '#123456',
          createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final c2 = await categories.insertCategory(Category(
          id: '', name: 'Alpha', colorHex: '#654321',
          createdAt: DateTime.now(), updatedAt: DateTime.now()));
