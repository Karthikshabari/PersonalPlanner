...
void main() {
  ...
  group('TaskRepository', () {
    test('insertTask assigns UUIDv7 id and timestamps', () async {
      ...
      final inserted = await taskRepo.insertTask(newTask(
        start: day,
    ...
    test('updateTask persists field changes', () async {
      ...
      await taskRepo.updateTask(inserted.copyWith(
        title: 'After',
    ...
    test('deleteTask soft-deletes and watchTasksForDay hides it',
        () async {
      final day = DateTime(2026, 7, 23, 10);
      final inserted = await taskRepo.insertTask(newTask(
        start: day,
      ...
      final before = await taskRepo.watchTasksForDay(DateTime(2026, 7, 23)).first;
      expect(before, hasLength(1));
      await taskRepo.deleteTask(inserted.id);
      final after = await taskRepo.watchTasksForDay(DateTime(2026, 7, 23)).first;
    ...
    test('watchTasksForDay returns only that day, excludes inbox',
        () async {
      final dayA = DateTime(2026, 7, 23, 10);
      final dayB = DateTime(2026, 7, 24, 10);
      await taskRepo.insertTask(newTask(
          title: 'On A',
          start: dayA,
      ...
      await taskRepo
          .insertTask(newTask(title: 'Inbox item', inbox: true));
      final stream =
          await taskRepo.watchTasksForDay(DateTime(2026, 7, 23)).first;
  ...
  group('CategoryRepository', () {
    test('seedDefaultsIfEmpty creates the 4 default categories once',
        () async {
      ...
      final workColor =
          categories.firstWhere((c) => c.name == 'Work').colorHex;
    ...
    test('category CRUD round-trip', () async {
      await categoryRepo.seedDefaultsIfEmpty();
      final created = await categoryRepo.insertCategory(Category(
        id: '',
        name: 'Side Project',
        colorHex: '#FF8800',
        sortOrder: 99,
