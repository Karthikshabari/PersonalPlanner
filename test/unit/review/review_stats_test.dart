...
void main() {
  ...
  Future<void> markWorkFocus() async {
    ...
    await (db.update(db.categories)..where((c) => c.id.equals(work.id)))
        .write(CategoriesCompanion(isFocus: const Value(true)));
  }

  group('ReviewRepository daily', () {
    test('save inserts then updates the same row per date', () async {
      final saved = await reviews.saveDailyReview(DailyReview(
        id: '',
        date: day,
    ...
    test('ratings are clamped to 1-5 and null passes through', () async {
      await reviews.saveDailyReview(DailyReview(
        id: '',
        date: day,
        energyLevel: 7,
    ...
    test('watch emits the saved review and soft-delete hides it', () async {
      ...
      final raw =
          await (db.select(db.dailyReviews)..where((r) => r.id.equals(saved.id)))
              .getSingle();
  ...
  group('ReviewRepository weekly', () {
    test('save inserts then updates the same row per week', () async {
      ...
      final saved = await reviews.saveWeeklyReview(WeeklyReview(
        id: '',
  ...
  group('DailyStatsService', () {
    test('computes counts, durations, focus hours and accuracy for a day',
        () async {
      await markWorkFocus();
      ...
      Future<Task> seed({
        ...
        int? actual,
          ...
          tasks.insertTask(Task(
            ...
            endTime: DateTime(day.year, day.month, day.day, startHour)
      ...
      await seed(title: 'Chores', startHour: 11, durationMin: 30,
          status: TaskStatus.completed);
      // Missed: planned and already over.
      await seed(title: 'Missed call', startHour: 13, durationMin: 60);
      // Terminal statuses.
      await seed(title: 'Skip it', startHour: 15, durationMin: 30,
          status: TaskStatus.skipped);
      await seed(title: 'Nope', startHour: 16, durationMin: 30,
          status: TaskStatus.cancelled);
      await seed(title: 'Moved', startHour: 17, durationMin: 30,
      ...
      final cached =
          await db.statsDao.getStatsForDate(isoDateString(day));
    ...
    test('review save trigger refreshes cached review-linked fields',
        () async {
      await reviews.saveDailyReview(DailyReview(
        id: '',
        date: day,
    ...
    test('range aggregation sums across days of a week', () async {
      ...
      await seedOn(DateTime(weekStart.year, weekStart.month, weekStart.day, 9), 30);
      await seedOn(
          DateTime(weekStart.year, weekStart.month, weekStart.day + 1, 10), 90);
    ...
    test('getTasksBetween excludes inbox items and deleted rows', () async {
      ...
      final kept = await tasks.insertTask(Task(
        id: '',
        title: 'Kept',
        startTime: at,
      ...
      final doomed = await tasks.insertTask(Task(
        id: '',
        title: 'Doomed',
        startTime: at,
        endTime: at.add(const Duration(hours: 1)),
