...
refreshCipher?:string;grantExpiresAt?:number;oauthAuthorizedAt?:number;oauthRevokedAt?:number;oauthReleaseUnconfirmed?:boolean;emailRedirectConfigured?:string};
 ...
 */
 ...
 if(current.oauthPurpose==="management"){if(current.oauthStateUsed!==true||!current.oauthStateHash||!same(h,current.oauthStateHash)||current.grantExpiresAt===undefined||now>=current.grantExpiresAt||current.oauthRevokedAt!==undefined)throw Error("oauth_expired");const refresh=await this.sealRefresh(refreshToken);this.save({...current,subject,oauthAuthorizedAt:now,oauthRevokedAt:undefined,refreshCipher:refresh??current.refreshCipher,oauthState:undefined,oauthStateHash:undefined,oauthVerifier:undefined,oauthStateUsed:undefined,oauthPurpose:undefined,updatedAt:now});
   // A re-authorization is the only moment this Worker holds a Management
   // access token for an already-READY backend, so it is also the only moment
   // it can add the email confirmation landing page to that project's Auth
   // redirect allow list. Success is recorded; failure only means the app keeps
   // its original scheme-based callback.
   const projectRef=current.projectRef;
   if(emailConfirmationRedirect!==undefined&&projectRef!==undefined&&validProjectRef(projectRef)){let configured=false;try{configured=await ensureAuthRedirectConfigured(projectRef,(path,init={})=>management(token,path,init),[emailConfirmationRedirect]);}catch{configured=false;}const live=this.load();if(live!==null&&live.oauthRevokedAt===undefined)this.save({...live,emailRedirectConfigured:configured?emailConfirmationRedirect:undefined,updatedAt:Date.now()});}
   // A re-authorization exists only to perform that management operation, so
   // the grant is released again immediately afterwards.
   await this.releaseManagementGrant();
   return;}
 ...
 async createContext(a:string){const t=await this.auth(a);return{state:t.state,projectRef:t.projectRef,organizationSlug:t.organizationSlug,requestedProjectName:t.requestedProjectName,operation:t.operation};}
...
export function managementAuthorizationCompletedPage():string{return browserPage({title:"Personal Planner",heading:"Supabase authorization completed",body:"Personal Planner can now continue with your cloud setup. Return to the app to finish the remaining steps.",actionHref:PLANNER_MANAGEMENT_CALLBACK_URI,actionLabel:"Open Personal Planner",autoOpenHref:PLANNER_MANAGEMENT_CALLBACK_URI,muted:"You can close this page after returning to Personal Planner."});}
/** The user declined, or Supabase rejected, the authorization request. */
export function managementAuthorizationDeniedPage():string{return browserPage({title:"Personal Planner",heading:"Supabase authorization was cancelled",body:"Nothing was created or changed. You can return to Personal Planner and start the authorization again whenever you are ready.",actionHref:PLANNER_MANAGEMENT_CALLBACK_URI,actionLabel:"Open Personal Planner",autoOpenHref:PLANNER_MANAGEMENT_CALLBACK_URI,muted:"You can close this page after returning to Personal Planner."});}
/** The authorization response could not be matched to a live request. */
export function managementAuthorizationInvalidPage():string{return browserPage({title:"Personal Planner",heading:"This authorization link is no longer valid",body:"The Supabase authorization could not be matched to a request from this device. Nothing was created or changed.",actionHref:PLANNER_MANAGEMENT_CALLBACK_URI,actionLabel:"Open Personal Planner",autoOpenHref:PLANNER_MANAGEMENT_CALLBACK_URI,muted:"You can close this page after returning to Personal Planner."});}
/** The authorization code could not be exchanged. No secret is ever shown. */
export function managementAuthorizationFailedPage():string{return browserPage({title:"Personal Planner",heading:"Supabase authorization could not be completed",body:"Personal Planner could not finish talking to Supabase. Nothing was created or changed, and no credentials were stored.",actionHref:PLANNER_MANAGEMENT_CALLBACK_URI,actionLabel:"Open Personal Planner",autoOpenHref:PLANNER_MANAGEMENT_CALLBACK_URI,muted:"You can close this page after returning to Personal Planner."});}
/**
...
export async function productionOAuthCallback(r:Request,env:Env):Promise<Response|null>{
  ...
  if(u.searchParams.get("error")!==null)return pageResponse(managementAuthorizationDeniedPage(),400);
  const code=u.searchParams.get("code");
  if(code===null||code.length===0||code.length>4096)return pageResponse(managementAuthorizationInvalidPage(),400);
  const d=tx(env,id);
  const claim=await d.oauthCallback(s);
  if(claim===null)return pageResponse(managementAuthorizationInvalidPage(),400);
  const basic=btoa(`${env.SUPABASE_OAUTH_CLIENT_ID}:${env.SUPABASE_OAUTH_CLIENT_SECRET}`),response=await fetch(`${API}/v1/oauth/token`,{method:"POST",headers:{authorization:`Basic ${basic}`,"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({code,code_verifier:claim.verifier,grant_type:"authorization_code",redirect_uri:env.SUPABASE_OAUTH_REDIRECT_URI}),signal:AbortSignal.timeout(15_000)});
  if(!response.ok){console.error(JSON.stringify({event:"oauth_token_exchange_failed",status:response.status}));return pageResponse(managementAuthorizationFailedPage(),502);}
  const p:unknown=await response.json();
  if(!record(p)||typeof p.access_token!=="string")return pageResponse(managementAuthorizationFailedPage(),502);
  ...
  const subject=typeof p.user_id==="string"?p.user_id:typeof p.sub==="string"?p.sub:"authorized";
  ...
  try{await d.saveOAuthFromCallback(s,p.access_token,refreshToken,Date.now()+expires*1000,subject,confirmationUri??undefined);}catch{console.error(JSON.stringify({event:"oauth_callback_not_live"}));return pageResponse(managementAuthorizationInvalidPage(),400);}
