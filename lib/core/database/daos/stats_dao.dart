...
class StatsDao extends DatabaseAccessor<AppDatabase> with _$StatsDaoMixin {
  ...
  Future<DailyStatsCacheRow?> getStatsForDate(String dateIso) =>
      (select(dailyStatsCache)..where((s) => s.date.equals(dateIso)))
          .getSingleOrNull();

  Stream<DailyStatsCacheRow?> watchStatsForDate(String dateIso) {
    return (select(dailyStatsCache)..where((s) => s.date.equals(dateIso)))
        .watchSingleOrNull();
  }

  Future<List<DailyStatsCacheRow>> getStatsBetween(
          String startIsoInclusive, String endIsoExclusive) =>
