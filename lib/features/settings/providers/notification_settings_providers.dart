...
class ReviewReminderEnabledNotifier extends AsyncNotifier<bool> {
  ...
  Future<bool> build() async {
    ...
    final row = await (db.select(db.appSettings)
          ..where((s) => s.key.equals(reminderEnabledKey)))
        .getSingleOrNull();
  ...
  Future<bool> _apply() async {
    ...
    final minutes = ref.read(reviewReminderMinutesProvider).value ??
        defaultReminderMinutes;
    return ref.read(notificationServiceProvider).scheduleDailyReminder(
          hour: minutes ~/ 60,
          minute: minutes % 60,
        );
  }
...
class ReviewReminderMinutesNotifier extends AsyncNotifier<int> {
  ...
  Future<int> build() async {
    final db = ref.watch(appDatabaseProvider);
    final row = await (db.select(db.appSettings)
          ..where((s) => s.key.equals(reminderTimeKey)))
        .getSingleOrNull();
  ...
  Future<void> setMinutes(int minutes) async {
    ...
    if (ref.read(reviewReminderEnabledProvider).value ?? true) {
      ...
      final scheduled = await ref.read(notificationServiceProvider).scheduleDailyReminder(
            hour: effective ~/ 60,
            minute: effective % 60,
          );
      if (!scheduled) {
