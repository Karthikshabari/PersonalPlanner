...
class TimerRepository {
  ...
  Stream<ActiveTimer?> watchActiveTimer() =>
      _dao.watchActiveTimerWithTask().map((row) => row == null
          ? null
          : ActiveTimer(
  ...
  Future<void> updateSession(TimerSession session) async {
    ...
    await _dao.updateSession(
      _toRow(session.copyWith(updatedAt: DateTime.now()),
          syncStatus: row.syncStatus, revision: row.revision),
    );
  }

  Future<void> deleteSession(String sessionId) =>
      _dao.softDeleteSession(sessionId, DateTime.now());
